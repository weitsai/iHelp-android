...
public class SendSMS {
    ...
    public SendSMS(Context context, String phone, String content, String location) {
        ...
        if (it.isConnectedOrConnecting()) {
            ...
            String url = SERVER_URL + "?MSGID=" + uid + "&OA=" + phone + "&SM=http://maps.google.com.tw/maps?q=" + location + "	" + content + "iHELP";

            String dataContent = "http://maps.google.com.tw/maps?q=" + location + "%0A" + content + "iHELP";
            Log.e("length" + dataContent.length(), "byte" + dataContent.getBytes().length);
            Log.e("url", url);
            String result = it.GetTo(url);
            Log.e("result", result);
        } else {
            Log.e("network", "offline");
            text = "http://maps.google.com.tw/maps?q=" + location + "%0a" + content + "&iHELP";
